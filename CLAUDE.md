...
- `src/fallbackPlayer/` - fallback playback for terminals that cannot run the full Ink player. `resolveFallbackRenderMode` picks the mode (a forced mode wins untouched, otherwise the kitty graphics probe decides, falling back to `detectCellRenderMode`, cell-background on Terminal.app, half-block elsewhere). Probe-free `createFallbackScreen` builds the Screen at that mode (full-screen, autoResize), and `runFallbackPlayer` is a React-free port of the playback clock with raw-stdin keys (space, arrows, q/Ctrl-C) and no Ink UI
